...
namespace App\Services\Excel\Configuration;
...
final class ExportService extends Service
{
    ...
    public function execute($identity): BinaryFileResponse
    {
        ...
        $spaceHeader = ['門牌', '稅籍號碼', '建號', '座落', '權利範圍', '建造執照號碼', '使用執照號碼', '主要用途', '土地使用分區'];
        ...
        $export = new Export();
        $export->condition([...['區名', '棟別', '梯間', '樓層', '戶別'], ...$header])
            ->appendCollectionData($data)
            ->asHorizontal(true)
            ->asVertical(true)
            ->asSelectMenu(true)
            ->appendSelectMenuParameter(self::fetchOption('privacy'));
    ...
    {
        return $this->crmBuildingSpaceRepository->findByAll()->map(
    ...
    {
        return $this->crmBuildingSpaceRepository->fetchFeeNumberExcel()->flatMap(function ($item) {
    ...
    private function mapSpaceFields($item): array
    {
        return [
            ...
            'main_application'          => CrmHouseType::array()[$item?->main_application] ?? null,
            'land_use_zoning'           => LandUseZoningType::array()[$item?->land_use_zoning] ?? null,
        ];
    }
