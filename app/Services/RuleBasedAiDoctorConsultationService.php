...
class RuleBasedAiDoctorConsultationService
{
    ...
    public function consult(Pet $pet, string $message, array $history = [], array $signals = []): array
    {
        $conversationSignals = $this->conversationSignals($history, $signals);
        $matchedCategories = $this->matchedCategories($conversationSignals);
        $categoryKey = $this->primaryCategoryKey($matchedCategories);
        $category = self::CATEGORY_RULES[$categoryKey];
        $severity = $this->severity($conversationSignals, $matchedCategories, $pet);
    ...
    private function conversationSignals(array $history, array $signals): array
    {
        $historySignals = collect($history)
            ->pluck('signals')
            ->flatten()
            ->filter(fn ($signal): bool => is_string($signal) && $signal !== '')
            ->all();

        return array_values(array_unique(array_merge($historySignals, $signals)));
    }

    private function matchedCategories(array $signals): array
    {
        $matches = [];

        foreach (self::CATEGORY_RULES as $key => $rule) {
            $matchedKeywords = array_values(array_intersect($rule['keywords'], $signals));
    ...
    private function severity(array $signals, array $matchedCategories, Pet $pet): string
    {
        foreach (self::EMERGENCY_KEYWORDS as $keyword) {
            if (in_array($keyword, $signals, true)) {
                return 'high';
            }
        }
