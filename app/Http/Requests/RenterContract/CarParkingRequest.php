...
class CarParkingRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        Arr::forget($rule, [
            ...
            'carpark',
            'carpark.*.type',
            'carpark.*.crmParkingSpaceId',
            'carpark.*.price',
            'carpark.*.licensePlateNumber',
            'itemsIncluded',
            'itemsIncluded.*'
        ]);
        ...
        $rule['associatedPersons.*.type'] = 'required|string|max:255|in:' . ContractPersonType::surety->name;
